...
## 7. Optional: Cloudflare OG worker (custom share URLs)

Static share presets live in `/share/{cityId}.html` (generated by `npm run seo`). For **custom** calculator URLs (`/index.html?city=…&salary=…`), social crawlers often miss dynamic OG tags on static GitHub Pages.

Optional fix — deploy `workers/og-share.js` on Cloudflare in front of `www.whereamirich.com`:

1. Upload `share-data.json` (from `npm run seo`) as a Worker asset or KV key `share-data`.
2. Create a Worker route for `www.whereamirich.com/*` with the script.
3. Bind `SHARE_KV` or serve `share-data.json` from the same origin.
4. Bots receive minimal HTML with OG tags; humans pass through to GitHub Pages.

Regenerate `share-data.json` whenever Numbeo indices change (`npm run seo`).
